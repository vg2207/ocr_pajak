            ...
            with st.empty():
                ...
                for image_path_in_colab in glob.glob(str(os.path.join(saved_directory+"/*.jpg"))):
                    ...
                    for i in range(len(df_all_data_extracted_combined)):
                        df_all_data_extracted_combined["CALCULATED TARIF"] = float(df_all_data_extracted_combined["PAJAK PENGHASILAN"][i].replace(".",""))/float(df_all_data_extracted_combined["DPP"][i].replace(".","").as)*100
